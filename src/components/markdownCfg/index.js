...
function mdInit() {
  ...
  mdHtml.renderer.rules.paragraph_open = mdHtml.renderer.rules.heading_open =
    injectLineNumbers;
}
...
const syncResultScroll = _.debounce(
  function () {
    const textarea = $('.source');
...
const syncSrcScroll = _.debounce(
  function () {
...
function loadPermalink() {
  ...
  try {
    if (/^#md3=/.test(location.hash)) {
      cfg = JSON.parse(
        mdurl.decode(location.hash.slice(5), mdurl.decode.componentChars)
      );
  ...
  _.forOwn(opts, function (val, key) {
    ...
    if (
      (_.isBoolean(defaults[key]) && _.isBoolean(val)) ||
...
$(function () {
  ...
  // Setup listeners
  $('.source').on(
    'keyup paste cut mouseup',
    _.debounce(updateResult, 300, { maxWait: 500 })
  );
